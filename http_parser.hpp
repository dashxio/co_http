...
struct http11_header_writer {
    ...
    size_t m_writed_bytes_num = 0;

    void reset_state() {
        m_writed_bytes_num = 0;
        m_buffer.clear();
    }

    [[nodiscard]] bool write_finished(size_t writed_this_time) {
        m_writed_bytes_num += writed_this_time;
        return m_writed_bytes_num >= m_buffer.size();
    }
    ...
    bytes_view buffer_need_to_write() {
        size_t remaining_bytes_num = m_buffer.size() - m_writed_bytes_num;
        return m_buffer.subspan(m_writed_bytes_num, remaining_bytes_num);
    }
...
struct _http_base_writer {
    ...
    [[nodiscard]] bool write_finished(size_t write_this_time) {
        return m_header_writer.write_finished(write_this_time);
    }
    ...
    bytes_view buffer_need_to_write() {
        return m_header_writer.buffer_need_to_write();
    }
